var App = (function() {
    ...
    var toggleFunction = {
        sidebar: function($recentSubmenu) {
            $(".sidebarCollapse").on("click", function(sidebar) {
                ...
                get_CompactSubmenuShow = document.querySelector(
                    "#compact_submenuSidebar"
                );
                get_overlay = document.querySelector(".overlay");
                get_mainContainer = document.querySelector(".main-container");
                if (
                    get_CompactSubmenuShow.classList.contains("show") ||
                    get_CompactSubmenuShow.classList.contains("hide-sub")
                ) {
                    console.log("main1");
                    ...
                    if (get_CompactSubmenuShow.classList.contains("hide-sub")) {
                        if (
                            get_mainContainer.classList.contains(
                                "sidebar-closed"
                            )
                        ) {
                            get_mainContainer.classList.remove(
                                "sidebar-closed"
                            );
        ...
        navbarShadow: function() {
            ...
            document.addEventListener("scroll", function() {
                ...
                if (top >= testDiv.offsetTop) {
                    getNav.style.boxShadow =
                        "0px 20px 20px rgba(126,142,177,0.12)";
                } else {
    ...
    var inBuiltfunctionality = {
        ...
        subCatScroll: function() {
            const submenuSidebar = new PerfectScrollbar(
                "#compact_submenuSidebar",
                {
                    wheelSpeed: 0.5,
                    swipeEasing: !0,
        ...
        onSidebarClick: function() {
            ...
            for (var i = 0; i < getMenu.length; i++) {
                getMenu[i].addEventListener("click", function() {
                    ...
                    getHref = this.querySelectorAll(
                        ".menu-toggle"
                    )[0].getAttribute("href");
                    getElement = document.querySelectorAll(
                        "#compact_submenuSidebar > " + getHref
                    )[0];
                    getMenuShowElement = document.querySelector(".menu.show");
                    getCompactSubmenu = document.querySelector(
                        "#compact_submenuSidebar"
                    );
                    getOverlayElement = document.querySelector(".overlay");
                    getElementActiveClass = document.querySelector(
                        "#compact_submenuSidebar > .show"
                    );
                    get_mainContainer = document.querySelector(
                        ".main-container"
                    );
        ...
        preventScrollBody: function() {
            $("#compactSidebar, #compact_submenuSidebar").bind(
    ...
    function sidebarFunctionality() {
        ...
        function sidebarMobCheck() {
            if (window.innerWidth <= 1199) {
                if (
                    $(".main-container").hasClass("sbar-open") ||
                    $("#compact_submenuSidebar").hasClass("show")
                ) {
                    return;
                } else {
                    sidebarCloser();
